...
function App() {
  ...
  const getData = async () => {
    if (blockchain.account !== "" && blockchain.smartContract !== null) {
      dispatch(fetchData(blockchain.account));
      const tokenId = await blockchain.smartContract.methods.tokenOfOwnerByIndex(blockchain.account, 0).call();
      let tokenMetadataURI = await blockchain.smartContract.methods.tokenURI(tokenId).call();
      ...
      console.log('tokenM', tokenMetadata)
    }
  };
    ...
    <s.Screen>
      ...
      >
        ...
        <ResponsiveWrapper flex={1} style={{ padding: 24 }} test>
          ...
          >
          {data.loading && <Circular color={'#ff1dec'} />}
          {nftMetadata && !data.loading ? (
            <>
            <s.TextTitle
            style={{
              textAlign: "center",
          ...
          >
            ...
            <StyledDefaultImg alt={"example"} src={`https://ipfs.io/ipfs/${nftMetadata.image.split('ipfs://')[1]}`} />
            </>
          ) : (
            <><s.TextTitle
            style={{
